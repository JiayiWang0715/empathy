...
def _natural_common_response(user_message, failures, understanding):
    ...
    if concern == "built_up_frustration":
        return (
            "That sounds really frustrating, especially if the dishes have been building up as a repeated issue. "
            "Snapping tonight does not mean your reaction came out of nowhere, but it may be a sign this has been bothering you for some time. "
            "Once things cool down, it might help to name the issue clearly without turning it into a fight."
        )

    if concern == "praise_rumination":
        return (
            "It makes sense that your mind keeps replaying that one awkward moment, especially when the praise mattered to you. "
            "One imperfect sentence probably does not erase the work your manager noticed. "
            "You might try letting the praise and the awkwardness both be true, instead of letting the awkward part take over the whole memory."
        )
    ...
    if concern == "small_but_meaningful_progress":
        return (
            "That does sound small and huge at the same time. "
            "When you have been avoiding something for weeks, cleaning your room can carry a lot more weight than the task itself. "
            "It is worth letting yourself register that progress instead of brushing it off."
        )
...
def _natural_subtle_response(user_message, failures, understanding):
    ...
    return (
        f"It sounds like {hint} affected you more than you may have had room to say in the moment. "
        "Even without naming the feeling directly, it makes sense if you are tense, worn down, or unsure what to do next. "
        "You might start by giving the feeling a little room instead of forcing it to be fine."
    )
