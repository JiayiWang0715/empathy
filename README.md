...
`app/planning/` contains a standalone, CoCoMo-inspired support planning layer
that converts understanding JSON into a structured `SupportPlan`. The plan
describes the support goal, response acts, constraints, safety notes, and repair
priorities that a future prompt builder can use.

This layer is not connected to `/chat` or runtime generation yet, and it does
not change model loading, the classifier, the generator, or the existing
understanding layer. To inspect example plans:
