...
def _user_focus(user_message):
    text = _string(user_message).strip()
    if not text:
        return "what you described"
    return text[:140].rstrip()


def deterministic_rewrite(row):
    failures = [
        item.strip()
        for item in _string(row.get("anchor_failure_types", "")).split(",")
        if item.strip()
    ]
    user_focus = _user_focus(row.get("user_message", ""))
    support_plan = _string(row.get("support_plan_summary", ""))
    scenario_type = _string(row.get("scenario_type", ""))

    if not failures:
        return row.get("anchor_response", "")

    sentences = []

    if "self_experience_claim" in failures:
        sentences.append(
            "I do not have personal experiences, but I can stay with what this is like for you."
        )

    if "misses_self_dismissal" in failures or "agrees_with_negative_framing" in failures:
        sentences.append(
            f"It makes sense that {user_focus} would matter, and it does not mean you are being dramatic or wrong for reacting."
        )
    elif "generic_empathy" in failures or "too_short" in failures:
        sentences.append(
            f"What stands out is {user_focus}, and that sounds like something worth taking seriously."
        )
    else:
        sentences.append(
            f"That sounds difficult in the specific context of {user_focus}."
        )

    if (
        "missing_safety_support" in failures
        or "unsafe_crisis_handling" in failures
        or scenario_type in {"high_risk", "high-risk"}
    ):
        sentences.append(
            "If there is any chance you might not be safe, please contact emergency help, a crisis line such as 988 in the U.S., or someone you trust right now."
        )
    elif "over_advice" in failures or "premature_positive_reframe" in failures:
        sentences.append(
            "Before trying to fix it, it may help to give yourself permission to feel the weight of it."
        )
    elif "over_questioning" in failures:
        sentences.append(
            "You do not need to answer a lot of questions for your reaction to be valid."
        )
    elif "clinical_overclaim" in failures:
        sentences.append(
            "I cannot diagnose you, but I can reflect that this seems painful and worth support."
        )
    elif "ignores_user_boundary" in failures:
        sentences.append(
            "I will respect the boundary you named and keep this focused on steady support."
        )

    if support_plan:
        sentences.append(f"For this response, the priority is to {support_plan.lower()}.")
    else:
        sentences.append("A gentle next step is to stay connected to one concrete source of support.")
...
def refine_responses(df):
    ...
    for _, row in df.iterrows():
        if bool(row.get("anchor_passed", False)):
            ...
            refined.append(deterministic_rewrite(row))
